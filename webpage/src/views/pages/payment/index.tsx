...
import { BillingRef } from './components/BillingTab';
import { CheckoutRef } from './components/CheckoutTab';
import { DetailsRef } from './components/DetailsTab';
import { WANumbersRef } from './components/WANumberTab';
...
const PaymentPage = () => {
	...
	const handlePaymentClick = async () => {
		...
		if (detailsRef.current?.getData()?.type === 'one-time' && result.razorpay_options) {
			...
			rzp1.open();
		} else if (detailsRef.current?.getData()?.type === 'subscription' && result.payment_link) {
		...
		<HStack pt={'2rem'} width={'full'} justifyContent={'space-around'}>
				...
				<Flex direction={'column'} gap={'1rem'}>
					...
					<HStack>
						...
						<Button
							...
							isLoading={loading}
						>
							{activeStep === 2 ? 'Checkout' : activeStep === 3 ? `Confirm` : 'Next'}
						</Button>
					</HStack>
				</Flex>
			</Box>
			...
			>
				...
				<AlertDialogContent>
					...
					<AlertDialogBody>
						Your payment has been successfull. Please check your email for the payment receipt.
					</AlertDialogBody>
					<AlertDialogFooter>
						<Button
							colorScheme='green'
			...
			>
				...
				<AlertDialogContent>
					...
					<AlertDialogBody>
						Please proceed to the payment page to complete the subscription. Your subscription will
						be activated once the payment is successfull.
					</AlertDialogBody>
					<AlertDialogFooter>
						<Button
							colorScheme='green'
