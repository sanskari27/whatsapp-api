...
export type SelectContactOrAttachmentListHandle = {
	...
	setAttachmentIds: (id: string[]) => void;
...
const SelectContactsOrAttachmentsList = forwardRef<SelectContactOrAttachmentListHandle, Props>(
	({ onConfirm }: Props, ref) => {
		...
		const [selectedAttachments, setAttachmentIds] = useState<string[]>([]);
		...
		useImperativeHandle(ref, () => ({
			...
			contactId,
			attachmentId: selectedAttachments,
		}));
									...
									{type === 'Attachments'
										? attachments.map((item, index) => (
												<Tr key={item.id}>
													<Td>
														<Checkbox
															isChecked={selectedAttachments.includes(item.id)}
															mr={4}
															onChange={(e) => {
																if (e.target.checked) {
																	setAttachmentIds([...contactId, item.id]);
																} else {
																	setAttachmentIds(contactId.filter((i) => i !== item.id));
																}
															}}
														/>
														{index + 1}
										...
										: list.map((item, index) => (
												<Tr key={item.id}>
													<Td>
														<Checkbox
															isChecked={contactId.includes(item.id)}
															mr={4}
															onChange={(e) => {
																if (e.target.checked) {
																	setContactId([...contactId, item.id]);
																} else {
																	setContactId(contactId.filter((i) => i !== item.id));
																}
															}}
														/>
														{index + 1}
