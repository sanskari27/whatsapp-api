		...
		>
			<Flex direction={'column'} gap={'0.5rem'}>
				<Flex alignItems='center' gap={'0.5rem'} mt={'1.5rem'}>
					<Icon height={6} width={6} as={RiRobot2Line} color={'green.400'} />
					<Text
						fontSize={'xl'}
				...
				<Flex direction={'column'} borderRadius={'20px'} mb={'1rem'} gap={2}>
					...
					<HStack alignItems={'start'}>
						...
						<FormControl isInvalid={!!uiDetails.attachmentError} flex={2}>
								...
								<Multiselect
									...
									onRemove={(selectedList: typeof allAttachments) =>
										...
										// })
										{
											...
											dispatch(setAttachments(selectedList.map((attachment) => attachment.id)));
										}
									}
									...
									ref={multiselectRef ? (ref) => (multiselectRef.current = ref) : null}
							...
							{uiDetails.attachmentError && (
								<FormErrorMessage>{uiDetails.attachmentError}</FormErrorMessage>
							)}
						</FormControl>
						<Box flex={2} width={'full'}>
							<Text className='text-gray-700 dark:text-gray-400'>Contact Cards</Text>
							<Button
								width={'full'}
								size={'sm'}
								variant={'outline'}
								...
								onClick={() => {
									...
									SelectContactsListRef.current?.setIds(shared_contact_cards);
								}}
								// isDisabled={!isAuthenticated}
							>
								Select Contact ({shared_contact_cards.length}) Selected
							</Button>
						</Box>
					</HStack>
