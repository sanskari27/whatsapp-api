...
export default function Bot() {
	...
	const contactAttachmentSelectRef = useRef<SelectContactOrAttachmentListHandle>(null);

	const { trigger, message, options, respond_to, shared_contact_cards, attachments } = useSelector(
	...
	const [uiDetails, setUiDetails] = useState<typeof initialUiState>(initialUiState);
	...
	const handleAddContactAndAttachment = () => {
		dispatch(setContactCards(contactAttachmentSelectRef.current?.contactId ?? []));
		dispatch(setAttachments(contactAttachmentSelectRef.current?.attachmentId ?? []));
	};
	...
	function handleSave() {
		...
		if (!message && attachments.length === 0 && shared_contact_cards.length === 0) {
			setUiDetails((prevState) => ({
				...prevState,
				messageError: 'Message or Attachment or Contact Card is required',
			}));
			return;
		}
		if (!respond_to) {
		...
		if (!trigger_details.trigger_gap_time || trigger_details.trigger_gap_time <= 0) {
			setUiDetails((prevState) => ({
				...prevState,
		...
		if (!delay_details.delay_gap_time || delay_details.delay_gap_time <= 0) {
			setUiDetails((prevState) => ({
				...prevState,
				...
				<Flex direction={'column'} borderRadius={'20px'} mb={'1rem'} gap={2}>
					...
					>
						...
						{uiDetails.triggerError && (
							<FormErrorMessage>{uiDetails.triggerError}</FormErrorMessage>
						)}
					</FormControl>

					<Flex gap={4}>
						<FormControl isInvalid={!!uiDetails.respondToError} flexGrow={1}>
							<Text className='text-gray-700 dark:text-gray-400'>Recipients</Text>
							<Select
								className={`!bg-[#ECECEC] dark:!bg-[#535353] rounded-md w-full ${
									respond_to ? ' text-black dark:text-white' : ' text-gray-700 dark:text-gray-400'
								}`}
								border={'none'}
								value={respond_to}
								onChange={(e) => {
									{
										...
										dispatch(
											setRespondTo(
												e.target.value as '' | 'ALL' | 'SAVED_CONTACTS' | 'NON_SAVED_CONTACTS'
											)
										);
									}
								}}
							...
							{uiDetails.respondToError && (
								<FormErrorMessage>{uiDetails.respondToError}</FormErrorMessage>
							)}
						</FormControl>
						<FormControl isInvalid={!!uiDetails.optionsError} flexGrow={1}>
							<Text className='text-gray-700 dark:text-gray-400'>Conditions</Text>
							<Select
								// isDisabled={!isAuthenticated}
								className={`!bg-[#ECECEC] dark:!bg-[#535353] rounded-md w-full ${
									options ? ' text-black dark:text-white' : ' text-gray-700 dark:text-gray-400'
								}`}
								border={'none'}
								value={options}
								onChange={(e) => {
							...
							{uiDetails.optionsError && (
								<FormErrorMessage>{uiDetails.optionsError}</FormErrorMessage>
							)}
						</FormControl>
					</Flex>
					<FormControl isInvalid={!!uiDetails.messageError}>
						...
						{uiDetails.messageError && (
							<FormErrorMessage>{uiDetails.messageError}</FormErrorMessage>
						)}
					</FormControl>

					<HStack alignItems={'start'}>
						<FormControl isInvalid={!!uiDetails.triggerGapError} flex={1}>
							...
							{uiDetails.triggerGapError && (
								<FormErrorMessage>{uiDetails.triggerGapError}</FormErrorMessage>
							)}
						</FormControl>
						<FormControl isInvalid={!!uiDetails.delayGapError} flex={1}>
							<Flex alignItems={'center'}>
						...
						<Box>
							...
							<Button
								...
								onClick={() => {
									contactAttachmentSelectRef.current?.open();
									contactAttachmentSelectRef.current?.setContactId(shared_contact_cards);
									contactAttachmentSelectRef.current?.setType('Contacts');
								}}
								// isDisabled={!isAuthenticated}
							>
					...
					<HStack justifyContent={'space-between'} alignItems={'center'} py={8}>
						<Button
							bgColor={uiDetails.editBot.isEditing ? 'red.300' : 'green.300'}
							width={'100%'}
							onClick={uiDetails.editBot.isEditing ? handleCancel : handleSave}
							isLoading={addingBot}
							// isDisabled={!isAuthenticated}
						>
							<Text color={'white'}>{uiDetails.editBot.isEditing ? 'Cancel' : 'Save'}</Text>
						</Button>
