    ...
    static async ListContactCards() {
        try {
            ...
            return data.contact_cards.map((card: any) => ({
                ...
                contact_details_phone: card.contact_details_phone
                    .contact_number && {
                    country_code:
                        card.contact_details_phone.contact_number.slice(
                            1,
                            -10
                        ) as string,
                    number: card.contact_details_phone.contact_number.slice(
                        -10
                    ) as string,
                },
                contact_details_work: card.contact_details_work
                    .contact_number && {
                    country_code:
                        card.contact_details_work.contact_number.slice(
                            1,
                            -10
                        ) as string,
                    number: card.contact_details_work.contact_number.slice(
                        -10
                    ) as string,
                },
                contact_details_other: card.contact_details_other.map(
                    (contact: { contact_number: string }) => ({
                        country_code: contact.contact_number.slice(
                            1,
                            -10
                        ) as string,
