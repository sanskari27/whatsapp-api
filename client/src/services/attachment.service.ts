...
export default class AttachmentService {
	...
	static async uploadAttachment(
		...
		try {
			...
			const { data } = await APIInstance.post(`/uploads/attachment`, formData, {
				headers: {
	...
	static async updateAttachmentDetails(attachment_details: {
		...
		try {
			const { data } = await APIInstance.patch(`/uploads/attachment/${attachment_details.id}`, {
	...
	static updateAttachmentFile(id: string, file: File, cb: (progress: number) => void) {
		const formData = new FormData();
		formData.append('file', file);
