...
const CSVFileSlice = createSlice({
    ...
    reducers: {
        ...
        addCsvFileList: (
            state,
            action: PayloadAction<(typeof initialState.list)[0]>
        ) => {
        ...
        findSelectedCSVFile: (state, action: PayloadAction<{ id: string }>) => {
            state.selectedCSV =
                state.list.find((csv) => csv.id === action.payload.id) ||
                initialState.selectedCSV;
        },
        ...
        setName: (
            state,
            action: PayloadAction<typeof initialState.selectedCSV.name>
        ) => {
            state.selectedCSV.name = action.payload;
        },
        setFile: (
            state,
            action: PayloadAction<typeof initialState.csvFile>
        ) => {
        ...
        setIsFetching: (
            state,
            action: PayloadAction<typeof initialState.uiDetails.isFetching>
        ) => {
            state.uiDetails.isFetching = action.payload;
        },
        setIsDeleting: (
            state,
            action: PayloadAction<typeof initialState.uiDetails.isDeleting>
        ) => {
            state.uiDetails.isDeleting = action.payload;
        },
        setIsCreating: (
            state,
            action: PayloadAction<typeof initialState.uiDetails.isCreating>
        ) => {
            state.uiDetails.isCreating = action.payload;
        },
        setIsUpdating: (
            state,
            action: PayloadAction<typeof initialState.uiDetails.isUpdating>
        ) => {
            state.uiDetails.isUpdating = action.payload;
        },
        setError: (
            state,
            action: PayloadAction<typeof initialState.uiDetails.error>
        ) => {
