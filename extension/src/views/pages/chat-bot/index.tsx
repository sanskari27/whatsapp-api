			...
			<Flex direction={'column'} gap={'0.5rem'}>
				...
				>
					<Flex direction={'column'} gap={2}>
						...
						<Input
							width={'full'}
							isInvalid={allBots.map((bot) => bot.trigger).includes(details.trigger)}
							...
							<Box>
								{details.shared_contact_cards.map((contact, index) => (
									...
									>
										...
										<TagCloseButton onClick={() => removeContact(contact.first_name ?? '')} />
									</Tag>
								))}
							</Box>
						</Box>
								...
								<Box>
									<Flex key={index} justifyContent={'space-between'}>
										<Box className='text-background-dark dark:text-background'>
											<Box className='flex flex-row'>
												<Box>
													{bot.trigger.length > 15 ? (
														<Box as='span'>{`${bot.trigger.substring(0, 15)}...`}</Box>
													) : (
														<Box as='span'>{bot.trigger}</Box>
													)}
													{' : '}
												</Box>
												<Box>
													<Box ml={'0.25rem'}>
														...
														{bot.message.length > 15 ? (
															<Box as='span'>{`${bot.message.substring(0, 15)}...`}</Box>
														) : (
															<Box as='span'>{bot.message}</Box>
														)}
											...
											<Box>
												{`${bot.respond_to}, ${bot.options}, ${
													bot.trigger_gap_seconds < 60
														? `${bot.trigger_gap_seconds} second${
																bot.trigger_gap_seconds !== 1 ? 's' : ''
														  }`
														: bot.trigger_gap_seconds < 3600
														? `${Math.floor(bot.trigger_gap_seconds / 60)} minute${
																Math.floor(bot.trigger_gap_seconds / 60) !== 1 ? 's' : ''
														  }`
														: bot.trigger_gap_seconds < 86400
														? `${Math.floor(bot.trigger_gap_seconds / 3600)} hour${
																Math.floor(bot.trigger_gap_seconds / 3600) !== 1 ? 's' : ''
														  }`
														: `${Math.floor(bot.trigger_gap_seconds / 86400)} day${
																Math.floor(bot.trigger_gap_seconds / 86400) !== 1 ? 's' : ''
														  }`
												}`}
											</Box>
										</Box>
