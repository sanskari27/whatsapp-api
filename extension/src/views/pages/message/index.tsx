...
const Message = () => {
	...
	const handleNextClick = () => {
		...
		if (activeStep === 1) {
			...
			if (!details.type) {
				...
				isError = true;
			} else if (details.type === 'GROUP' && details.group_ids.length === 0) {
				isError = true;
			} else if (details.type === 'GROUP_INDIVIDUAL' && details.group_ids.length === 0) {
				isError = true;
			} else if (details.type === 'LABEL' && details.label_ids.length === 0) {
				isError = true;
			} else if (details.type === 'CSV' && details.csv_file === 'select') {
				isError = true;
			}
			if (isError) {
				setUIDetails((prev) => ({
	...
	const removeContact = (contact: {
		...
		setDetails((prevState) => ({
			...prevState,
			shared_contact_cards: prevState.shared_contact_cards.filter((data) => data !== contact),
		}));
	};
				...
				<Stepper index={activeStep} size={'sm'} mx={'0.5rem'} colorScheme='green'>
					{steps.map((step, index) => (
						<Step key={index}>
							...
							<Box flexShrink='0'>
								<StepTitle className='text-gray-700 dark:text-gray-400'>{step}</StepTitle>
							</Box>
			...
			{!isAuthenticated ? (
				...
				<Flex justifyContent={'space-between'} alignItems={'center'}>
					...
					<Button
						...
						isLoading={uiDetails.recipientsLoading || uiDetails.schedulingMessages}
