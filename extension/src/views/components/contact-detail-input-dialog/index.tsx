...
const ContactDetailInputDialog = ({
	...
	const addContactOthers = () => {
		...
		setContact({
			...contact,
			contact_number_other: [...contact.contact_number_other, `${OTHERS.code}${OTHERS.number}`],
		});
		setContactNumber({
			...contactNumber,
			OTHERS: {
	...
	const handleSave = () => {
		onConfirm({
			...
			contact_number_phone: PHONE.number ? `${PHONE.code}${PHONE.number}` : '',
			contact_number_work: WORK.number ? `${WORK.code}${WORK.number}` : '',
			...
			>
				...
				<ModalBody className='custom-scrollbar'>
					...
					<HStack alignItems={'end'}>
						<Box width={'full'}>
							<Text color={'#7D7D7D'}>Website and Social Media Links</Text>
							<Input
								type='url'
					...
					<Box>
						{contact.links.map((link, index) => (
							...
							>
								...
								<TagCloseButton onClick={() => removeContactLinks(link)} />
							</Tag>
						))}
					</Box>
					<HStack>
					...
					<Box>
						...
						<InputGroup size={'sm'} bgColor={'transparent'}>
							<InputLeftAddon
								...
								children={
									<CountryCodeInput
										value={PHONE.code}
										onChange={(text) => {
											updateContactNumber('PHONE', 'code', text);
										}}
									/>
								}
							/>
							<Input
								...
								name='number'
								onChange={(e) => {
									updateContactNumber('PHONE', 'number', e.target.value);
								}}
							/>
						</InputGroup>
					</Box>
					<Box>
						...
						<InputGroup size={'sm'}>
							<InputLeftAddon
								...
								children={
									<CountryCodeInput
										value={WORK.code}
										onChange={(text) => {
											updateContactNumber('WORK', 'code', text);
										}}
									/>
								}
							/>
							...
							<Input
								...
								value={WORK.number}
								onChange={(e) => {
									updateContactNumber('WORK', 'number', e.target.value);
								}}
							/>
						</InputGroup>
					</Box>
						...
						<Box width={'full'}>
							...
							<InputGroup size={'sm'}>
								<InputLeftAddon
									...
									children={
										<CountryCodeInput
											value={OTHERS.code}
											onChange={(text) => {
												updateContactNumber('OTHERS', 'code', text);
											}}
										/>
									}
								/>
								<Input
									...
									value={OTHERS.number}
									onChange={(e) => {
										updateContactNumber('OTHERS', 'number', e.target.value);
									}}
								/>
							</InputGroup>
						</Box>
					...
					<Box>
						{contact.contact_number_other.map((number, index) => (
							...
							>
								...
								<TagCloseButton onClick={() => removeContactOthers(number)} />
							</Tag>
						))}
					</Box>
					<Box>
				...
				<ModalFooter>
					<Button width={'full'} colorScheme='green' onClick={handleSave} size={'sm'}>
						Add Contact
					</Button>
				</ModalFooter>
			</ModalContent>
